...
exports.serialize = function (mapper, object, objectName, client) {
  ...
  if (mapperType.match(/^(Number|String|Boolean)$/ig) !== null) {
    ...
    payload = serializeDateTypes(mapperType, null, object);
...
function serializeSequenceType(mapper, object, objectName, client) {
  ...
  for (var i = 0; i < object.length; i++) {
    tempArray[i] = exports.serialize(mapper.element, object[i], objectName, client);
  }
  return tempArray;
}
...
function serializeDictionaryType(mapper, object, objectName, client) {
  ...
  for (var key in object) {
    tempDictionary[key] = exports.serialize(mapper.value, object, objectName, client);
  }
...
function serializeCompositeType(mapper, object, objectName, client) {
  if (object !== null && object !== undefined) {
    ...
    if (!modelProps) {
      throw new Error(util.format('modelProperties cannot be null or undefined in the mapper \'%s\' of type \'%s\'',
        JSON.stringify(mapper), mapper.type.className));
    }
    for (var key in modelProps) {
      ...
      if (object[key] !== null && object[key] !== undefined) {
        var propertyObjectName = objectName + '.' + objectNameFromSerializedName(mapper.serializedName);
        var propertyMapper = modelProps[key].type;
        ...
        if (modelProps[key].type.name === 'Composite') {
          ...
          propertyMapper = client._models[modelProps[key].type.className].mapper();
          if (!propertyMapper) {
            throw new Error(util.format('mapper() cannot be null or undefined for model \'%s\'', 
              modelProps[key].type.className));
          }
        }
        return exports.serialize(client, propertyMapper, object[key], propertyObjectName);
      }
    }
  }
  return object;
...
function serializeDateTypes(typeName, objectName, value) {
  if (typeName === 'Date') {
