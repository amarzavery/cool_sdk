...
StorageAccounts.prototype.create = function (resourceGroupName, accountName, parameters, options, callback) {
  ...
  try {
    args = Args([
      { resourceGroupName: Args.STRING | Args.Required },
      { accountName: Args.STRING | Args.Required },
      { parameters: Args.OBJECT | Args.Required },
      { options: Args.OBJECT | Args.Optional },
      { callback: Args.FUNCTION | Args.Required }
    ], arguments);
  } catch (err) {
    if (err.message.indexOf('callback') > 0 || typeof arguments[arguments.length - 1] !== 'function') {
      throw new Error('callback cannot be null.');
    } else {
      callback(err);
    }
