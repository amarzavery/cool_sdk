...
StorageAccounts.prototype.checkNameAvailability = function (accountName, options, callback) {
  ...
  //var param = client._models['StorageAccountCheckNameAvailabilityParameters'];
  var resultModel = new client._models['StorageAccountCheckNameAvailabilityParameters'](accountName);
  try {
...
StorageAccounts.prototype.beginCreate = function (resourceGroupName, accountName, parameters, options, callback) {
  ...
  // Serialize Request
  var requestContent = null;
  var resultModel = new client._models['StorageAccountCreateParameters'](parameters);
  try {
    requestContent = JSON.stringify(resultModel.serialize());
  } catch (error) {
    var serializationError = new Error(util.format('Error "%s" occurred in serializing the payload - "%s"', error, util.inspect(resultModel, { depth: null })));
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    result.closure = function (httpRequest, response) {
      var responseBody = response.body;
      if (responseBody === '') responseBody = null;
      // Deserialize Response
      var resultModel = null;
      if (response.statusCode === 200) {
        resultModel = new client._models['StorageAccount']();
        var parsedResponse = null;
        try {
          parsedResponse = JSON.parse(responseBody);
          if (parsedResponse !== null && parsedResponse !== undefined) {
            resultModel.deserialize(parsedResponse);
          }
        } catch (error) {
          var deserializationError = new Error(util.format('Error "%s" occurred in deserializing the responseBody - "%s"', error, responseBody));
          deserializationError.request = httpRequest;
